...
collect<-function(results_path="./output/results/"){
	...
	for (i in list.files(results_path, pattern="*.rds")){
	  ...
	  tmp_data=data.frame(id=c,hearing=tmp$results$Hearing,sex=tmp$results$Gender,Age=tmp$results$Age);
...
collect_spread<-function(results_path="./output/results/"){
	...
	for (i in list.files(results_path, pattern="*.rds")){
	  ...
	  for (j in names(subject)[seq(2,length(subject)-1)]) {
	    ...
	    n=nrow(trajectory);
	    data=rbind(data,data.frame(ids=ids,hearing=subject$results$Hearing,genre=subject$results$Genre,age=subject$results$Age,trajectory=j,idx=seq(0,n-1),X=trajectory[,1],Y=trajectory[,2],Z=trajectory[,3]));
	  }
	  ids=ids+1;
	}
	return(data);
